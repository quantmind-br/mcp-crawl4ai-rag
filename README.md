...
# Chat Model for summaries and contextual embeddings
...
# If a primary model fails, the server will use these after retries.
# Leave blank if you don't want to use a fallback.
CHAT_MODEL_FALLBACK=
...
# --- Retry Configuration ---
CHAT_MODEL_RETRIES=2
EMBEDDING_MODEL_RETRIES=2

# --- Other RAG & Performance Configuration ---
...
### Performance and Model Configuration

- **`EMBEDDING_MODEL`**: Specifies the model used for generating embeddings (default: `text-embedding-3-small`).
- **`EMBEDDING_DIMENSION`**: The dimension of the embeddings, corresponding to the `EMBEDDING_MODEL` (default: `1536`).
- **`RERANKING_MODEL`**: The cross-encoder model used for reranking search results when `USE_RERANKING` is enabled (default: `cross-encoder/ms-marco-MiniLM-L-6-v2`).
- **`MAX_WORKERS`**: The maximum number of concurrent threads for tasks like generating summaries and embeddings (default: `10`).
- **`SUPABASE_BATCH_SIZE`**: The number of records to batch together for insertions into Supabase (default: `20`).
