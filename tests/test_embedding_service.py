...
class TestSparseVectorEncoder:
    ...
    def test_encode_batch_success(self, mock_sparse_embedding):
        ...
        mock_encoder_instance.embed.return_value = [mock_sparse_result1, mock_sparse_result2]
        mock_sparse_embedding.return_value = mock_encoder_instance
    ...
    def test_encode_fastembed_not_available(self):
        ...
        with patch('fastembed.SparseTextEmbedding', side_effect=ImportError("FastEmbed not found")):
            with pytest.raises(ImportError, match="FastEmbed is required for sparse vectors"):
...
class TestEmbeddingService:
    ...
    def test_create_embedding_success(self):
        ...
        with patch.object(service, 'create_embeddings_batch', return_value=[[0.1, 0.2, 0.3]]):
    ...
    def test_create_embedding_error(self, mock_get_dimensions):
        ...
        with patch.object(service, 'create_embeddings_batch', side_effect=Exception("API Error")):
            result = service.create_embedding("test text")
    ...
    @patch('src.services.embedding_service.get_embeddings_client')
    @patch('fastembed.SparseTextEmbedding')
    def test_create_embeddings_batch_hybrid(self, mock_sparse_embedding, mock_get_client):
        ...
        dense_vectors, sparse_vectors = service.create_embeddings_batch(["text1", "text2"])
...
class TestGlobalFunctions:
    ...
    def test_create_sparse_embeddings_batch_convenience(self, mock_get_service):
        ...
        mock_service.create_sparse_embeddings_batch.assert_called_once_with(["text1", "text2"])
    ...
    def test_generate_contextual_embedding_convenience(self, mock_get_service):
        ...
        mock_service = Mock()
        mock_service.generate_contextual_embedding.return_value = ("contextual text", True)
        mock_get_service.return_value = mock_service
        
        result = generate_contextual_embedding("full doc", "chunk")
        
        assert result == ("contextual text", True)
        mock_service.generate_contextual_embedding.assert_called_once_with("full doc", "chunk")
...
class TestErrorHandling:
    ...
    @patch('src.services.embedding_service.get_embeddings_client')
    @patch('src.services.embedding_service.get_embedding_dimensions')
    def test_individual_fallback_on_batch_failure(self, mock_get_dimensions, mock_get_client):
        ...
        mock_client.embeddings.create.side_effect = batch_side_effects + individual_responses
        mock_get_client.return_value = mock_client
