...
class TestTimeoutBehavior:
    ...
    def test_timeout_constants_environment_override(self, timeout_env):
        ...
        with patch.dict(os.environ, timeout_env, clear=False):
            ...
            from src.core.app import QUICK_TIMEOUT, MEDIUM_TIMEOUT, LONG_TIMEOUT, VERY_LONG_TIMEOUT
    ...
    def test_tool_registration_with_custom_timeouts(self, mock_app, timeout_env):
        ...
        with patch.dict(os.environ, timeout_env, clear=False):
            ...
            with patch('src.tools.web_tools') as mock_web_tools, \
                ...
                timeout_calls = [call for call in mock_app.tool.call_args_list 
                ...
                assert 20 in timeout_values  # VERY_LONG_TIMEOUT for smart_crawl_url, index_github_repository
    ...
    def test_invalid_timeout_environment_handling(self):
        ...
        with patch.dict(os.environ, invalid_env, clear=False):
            ...
            with pytest.raises(ValueError):
                from src.core.app import QUICK_TIMEOUT
    ...
    def test_timeout_configuration_completeness(self, mock_app):
        ...
        with patch('src.tools.web_tools') as mock_web_tools, \
            ...
            with patch.dict(os.environ, {"USE_AGENTIC_RAG": "true", "USE_KNOWLEDGE_GRAPH": "true"}):
                register_tools(mock_app)
            ...
            assert len(timeout_calls) >= 8, f"Expected at least 8 tools with timeout, got {len(timeout_calls)}"
    ...
    def test_timeout_hierarchy_validation(self):
        ...
        with patch.dict(os.environ, default_env, clear=False):
            ...
            assert QUICK_TIMEOUT <= MEDIUM_TIMEOUT, f"QUICK ({QUICK_TIMEOUT}) should be <= MEDIUM ({MEDIUM_TIMEOUT})"
            assert MEDIUM_TIMEOUT <= LONG_TIMEOUT, f"MEDIUM ({MEDIUM_TIMEOUT}) should be <= LONG ({LONG_TIMEOUT})"
            assert LONG_TIMEOUT <= VERY_LONG_TIMEOUT, f"LONG ({LONG_TIMEOUT}) should be <= VERY_LONG ({VERY_LONG_TIMEOUT})"
    ...
    def test_partial_environment_configuration(self):
        ...
        with patch.dict(os.environ, partial_env, clear=False):
            ...
            for key in ['MCP_MEDIUM_TIMEOUT', 'MCP_LONG_TIMEOUT', 'MCP_VERY_LONG_TIMEOUT']:
