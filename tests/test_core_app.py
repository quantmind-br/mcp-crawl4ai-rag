...
class TestKnowledgeGraphSingleton:
    ...
    def test_singleton_pattern(self):
        """Test that KnowledgeGraphSingleton follows singleton pattern."""
        from src.core.app import KnowledgeGraphSingleton
        
        instance1 = KnowledgeGraphSingleton()
        instance2 = KnowledgeGraphSingleton()
        
        assert instance1 is instance2

    @patch.dict(os.environ, {"USE_KNOWLEDGE_GRAPH": "false"})
    @pytest.mark.asyncio
    async def test_get_components_when_disabled(self):
        """Test get_components_async returns None when disabled."""
        from src.core.app import KnowledgeGraphSingleton
        
        singleton = KnowledgeGraphSingleton()
        validator, extractor = await singleton.get_components_async()
...
class TestApplicationLifecycle:
    ...
    async def test_crawl4ai_lifespan_success(
        self, mock_kg_singleton, mock_rerank_singleton, mock_validate_config,
        mock_get_cache, mock_get_client, mock_crawler_class
    ):
...
class TestToolRegistration:
    ...
    @patch("src.tools.web_tools")
    @patch("src.tools.github_tools") 
    @patch("src.tools.rag_tools")
    def test_register_tools_basic(self, mock_rag_tools, mock_github_tools, mock_web_tools):
    ...
    @patch("src.tools.github_tools")
    @patch("src.tools.rag_tools")
    def test_register_tools_with_agentic_rag(self, mock_rag_tools, mock_github_tools, mock_web_tools):
    ...
    def test_register_tools_with_knowledge_graph(
        ...
        with patch.dict(os.environ, {"USE_AGENTIC_RAG": "false", "USE_KNOWLEDGE_GRAPH": "true"}):
            register_tools(mock_app)
        
        # Verify KG tool registration
        assert mock_app.tool.call_count >= 7  # At least 7 tools including KG tools