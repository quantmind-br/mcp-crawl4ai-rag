...
class HybridSearchBenchmark:
    ...
    def benchmark_collection_creation(self, collection_name: str, use_hybrid: bool) -> Dict[str, float]:
    ...
    def benchmark_insertion_performance(self, collection_name: str, documents: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def benchmark_search_performance(self, collection_name: str, query_vector: List[float], test_types: List[str]) -> Dict[str, Dict[str, float]]:
        ...
        for test_type in test_types:
            ...
            if test_type == "semantic_only":
                ...
                sparse_queries = [SparseVector(indices=[1, 2, 3], values=[1.0, 0.8, 1.2])] * 5
                ...
                for _ in range(5):  # Fewer iterations for batch tests
                    ...
                    try:
                        ...
                        self.wrapper.client.search_batch(
                            collection_name=collection_name,
                            requests=[
                                {"name": "text-dense", "vector": dense_queries[0], "limit": 10},
                                {"name": "text-sparse", "vector": sparse_queries[0], "limit": 10}
                            ]
                        )
                        times.append(time.time() - start)
                    except Exception as e:
    ...
    def run_comprehensive_benchmark(self, document_counts: List[int] = None) -> Dict[str, Any]:
        ...
        for count in document_counts:
            ...
            try:
                ...
                for use_hybrid in [True, False]:
                    ...
                    creation_results[use_hybrid] = self.benchmark_collection_creation(col_name, use_hybrid)
                
                # Benchmark insertion (only for hybrid collections for efficiency)
                insertion_results = self.benchmark_insertion_performance(hybrid_col, documents)
                
                # Benchmark various search types
                search_results = self.benchmark_search_performance(hybrid_col, query_vector, [
                ...
                report["results"][f"{count}_docs"] = {
                    ...
                    "memory_usage": {
                        "per_doc_bytes": len(json.dumps(documents[0])) if documents else 0
                    }
                }
    ...
    def generate_performance_report(self, benchmark_results: Dict[str, Any]) -> str:
        ...
        for count, results in benchmark_results["results"].items():
            ...
            if insertion.get("success"):
                report.append(f"  📥 Insertion: {insertion['docs_per_second']:.1f} docs/sec")
            ...
            for test_type, perf_data in search.items():
                if "avg_time" in perf_data:
                    ...
                    report.append(f"      ({perf_data['min_time']:.4f}s - {perf_data['max_time']:.4f}s)")
