...
class PerformanceMetrics:
    ...
    def stop_monitoring(self):
        ...
        return {
            ...
            'peak_cpu': max((s['cpu_total'] for s in self.cpu_usage_samples), default=0),
            'avg_cpu': sum(s['cpu_total'] for s in self.cpu_usage_samples) / len(self.cpu_usage_samples) if self.cpu_usage_samples else 0,
            'peak_memory_mb': max((s['memory_mb'] for s in self.memory_usage_samples), default=0),
    ...
    def _analyze_multi_core_usage(self) -> Dict[str, Any]:
        ...
        for sample in self.cpu_usage_samples:
            ...
            variance = sum((usage - avg_usage) ** 2 for usage in core_usages) / len(core_usages)
            utilization_spread = max(utilization_spread, variance)
        
        return {
            'cores_used': cores_used,
            'utilization_spread': utilization_spread,
            'max_core_usage': max(max(s['cpu_per_core']) for s in self.cpu_usage_samples)
        }
...
class TestConcurrentPerformanceImprovement:
    ...
    async def test_concurrent_vs_sequential_reranking(self):
        ...
        try:
            ...
            test_results = [[{"content": f"doc_{i}_{j}"} for j in range(3)] for i in range(4)]
    ...
    async def test_mixed_workload_responsiveness(self):
        ...
        try:
            ...
            async def heavy_task():
                ...
                return await service.rerank_results_async("heavy query", results, executor)
            ...
            assert all(result == "lightweight_response" for result in lightweight_results)
            ...
            assert multi_core_data['cores_used'] >= 2, "Should utilize multiple CPU cores"
    ...
    async def test_worker_scaling_performance(self):
        ...
        for worker_count in [2, 4, 8]:
            ...
            try:
                ...
                print(f"Workers: {worker_count}, Time: {execution_time:.2f}s, Throughput: {tasks_count/execution_time:.2f} tasks/s")
        ...
        for workers, data in results_by_workers.items():
            if workers > base_workers:
                ...
                assert efficiency_ratio > 0.3, f"Poor scaling efficiency: {efficiency_ratio:.2f}"
...
class TestRealWorldScenarios:
    ...
    async def test_typical_rag_query_performance(self):
        ...
        qdrant_client = Mock()
        reranker = create_mock_reranker(processing_time=0.05)  # Realistic reranking time
        ...
        try:
            ...
            service.search_documents = Mock(return_value=[
                {"content": f"Document {i} content with relevant information", "score": 0.7 + i * 0.01}
            ...
            assert perf_data['total_time'] < 2.0, f"Queries took too long: {perf_data['total_time']:.2f}s"
    ...
    async def test_burst_request_handling(self):
        ...
        qdrant_client = Mock()
        reranker = create_mock_reranker(processing_time=0.1)  # Longer processing time to show multi-core usage
        ...
        try:
            ...
            multi_core_data = perf_data['multi_core_utilization']
            assert multi_core_data['cores_used'] >= 2, "Should use multiple cores for burst handling"
...
class TestPerformanceRegressionDetection:
    ...
    async def test_response_time_targets(self):
        ...
        try:
            ...
            assert response_time < 0.5, f"Response time too slow: {response_time*1000:.1f}ms"
    ...
    def test_resource_utilization_efficiency(self):
        ...
        try:
            ...
            assert memory_overhead < 50, f"High memory overhead: {memory_overhead:.1f}MB"
