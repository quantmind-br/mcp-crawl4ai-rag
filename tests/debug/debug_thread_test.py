...
"""
        ...
        result = await service._process_file_for_neo4j(
            test_file_path, go_content, "test-repo:test.go", request
        )
        ...
        if hasattr(service, "_neo4j_analyses"):
            logger.info(
                f"THREAD TEST - Number of analyses collected: {len(service._neo4j_analyses)}"
            )
            for i, analysis in enumerate(service._neo4j_analyses):
                logger.info(
                    f"THREAD TEST - Analysis {i}: Functions={len(analysis.get('functions', []))}"
                )
        else:
            logger.warning(
                "THREAD TEST - No _neo4j_analyses attribute found after threading!"
            )
