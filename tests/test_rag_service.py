...
class TestRagService:
    ...
    def test_search_code_examples_success(self, mock_create_embedding):
        ...
        # Execute
        results = self.rag_service.search_code_examples("test query", source_id="github.com")
        
        # Verify
        assert results == expected_results
        # Check that enhanced query was created
        expected_query = "Code example for test query\n\nSummary: Example code showing test query"
    ...
    def test_update_source_info_error(self):
        ...
        self.mock_qdrant_client.update_source_info.side_effect = Exception("Update error")
    ...
    def test_hybrid_search_disabled_documents(self):
        ...
        with patch.object(self.rag_service, 'search_documents', return_value=expected_results) as mock_search:
            # Execute
    ...
    def test_hybrid_search_enabled_code_examples(self):
        ...
        self.mock_qdrant_client.hybrid_search_code_examples.return_value = expected_results
        
        # Execute
        results = self.rag_service.hybrid_search("test query", search_type="code_examples")
    ...
    def test_rerank_results_success(self, mock_cleanup):
        ...
        expected_pairs = [["test query", "first result"], ["test query", "second result"]]
    ...
    def test_search_with_reranking_hybrid_enabled(self):
        ...
        with patch.object(self.rag_service, 'hybrid_search') as mock_hybrid, \
             patch.object(self.rag_service, 'rerank_results') as mock_rerank:
            
            mock_hybrid.return_value = [{"id": "1", "content": "test"}]
            mock_rerank.return_value = [{"id": "1", "content": "test", "rerank_score": 0.8}]
    ...
    def test_fuse_search_results(self):
        ...
        # Execute
        fused = self.rag_service.fuse_search_results(dense_results, sparse_results, k=60)
