...
class TestMainEntryPoint:
    ...
    @patch('src.__main__.setup_event_loop')
    def test_main_entry_point_imports_order(self, mock_setup_event_loop, mock_run_server):
    ...
    @patch('asyncio.run')
    def test_main_execution_flow(self, mock_asyncio_run, mock_setup_event_loop, mock_run_server):
        ...
        mock_setup_event_loop.assert_called_once()
...
class TestEventLoopConfiguration:
    ...
    def test_playwright_detection_after_imports(self):
        ...
        # Now Playwright should be detected
    ...
    def test_event_loop_setup_after_refactoring(self):
        ...
        # Should detect Playwright and use ProactorEventLoop on Windows
        if sys.platform == "win32":
            assert info['playwright_detected'] is True
            assert info['current_policy'] == 'WindowsProactorEventLoopPolicy'
    ...
    def test_windows_proactor_event_loop_policy(self):
        ...
        assert policy.__class__.__name__ == 'WindowsProactorEventLoopPolicy'
    ...
    def test_event_loop_validation_info(self):
        ...
        required_fields = [
            'platform', 'current_policy', 'is_windows',
            'playwright_detected', 'fix_applied', 'recommendations'
        ]
...
class TestApplicationInitialization:
    ...
    async def test_application_startup_sequence(
        self, mock_validate_config, mock_get_cache, mock_get_client, mock_crawler_class
    ):
