...
class TestToolRegistration:
    ...
    def test_timeout_configuration_applied(
        ...
        assert len(timeout_calls) > 0, (
            "No timeout parameters found in tool registrations"
        )
        ...
        assert (
            len(
                [
                    ...
                    or (call.args and any("timeout" in str(arg) for arg in call.args))
                ]
            )
            >= 8
    ...
    def test_web_tools_timeout_values(self, mock_web_tools):
        ...
        assert LONG_TIMEOUT in timeout_values, (
            f"LONG_TIMEOUT ({LONG_TIMEOUT}) not found in timeouts: {timeout_values}"
        )
        assert VERY_LONG_TIMEOUT in timeout_values, (
            f"VERY_LONG_TIMEOUT ({VERY_LONG_TIMEOUT}) not found in timeouts: {timeout_values}"
        )
    ...
    def test_rag_tools_timeout_values(self, mock_rag_tools):
        ...
        assert QUICK_TIMEOUT in timeout_values, (
            f"QUICK_TIMEOUT ({QUICK_TIMEOUT}) not found in timeouts: {timeout_values}"
        )
        assert MEDIUM_TIMEOUT in timeout_values, (
            f"MEDIUM_TIMEOUT ({MEDIUM_TIMEOUT}) not found in timeouts: {timeout_values}"
        )
