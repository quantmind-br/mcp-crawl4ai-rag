...
class TestRagFileIdIntegration:
    ...
    def test_add_documents_with_file_ids(self, mock_create_embeddings):
        ...
        url_to_full_document = {"https://github.com/test/repo": "Full document content"}
        ...
        self.mock_qdrant_client.add_documents_to_qdrant.return_value = (
            mock_point_batches
        )
    ...
    def test_add_code_examples_with_file_ids(self, mock_create_embeddings):
        ...
        self.mock_qdrant_client.add_code_examples_to_qdrant.return_value = (
            mock_point_batches
        )
...
class TestNeo4jFileIdIntegration:
        ...
        mock_results = [
            {
                ...
                "c": {"name": "MainClass", "line_number": 10},
            }
        ]
    ...
    def test_neo4j_cross_file_relationships_with_file_ids(self):
        ...
        with patch("neo4j.GraphDatabase.driver", return_value=self.mock_neo4j_driver):
            ...
            self.mock_session.run(
                query, source_file_id=source_file_id, target_file_id=target_file_id
            )
...
class TestCrossSystemLinking:
    ...
    def test_rag_to_neo4j_linking_scenario(self):
        ...
        with patch("neo4j.GraphDatabase.driver", return_value=self.mock_neo4j_driver):
            ...
            rag_content = self.rag_service.search_with_reranking(
                query="Agent class implementation", filter_metadata={"file_id": file_id}
            )
            ...
            self.mock_neo4j_session.run(structure_query, file_id=extracted_file_id)
            ...
            self.mock_neo4j_session.run.assert_called_once_with(
                structure_query, file_id=file_id
            )
    ...
    def test_neo4j_to_rag_linking_scenario(self):
        ...
        with patch("neo4j.GraphDatabase.driver", return_value=self.mock_neo4j_driver):
            ...
            self.mock_neo4j_session.run(class_query, class_name="FastAPI")
            ...
            assert extracted_file_id == file_id
            self.rag_service.search_with_reranking.assert_called_once()
            filter_used = self.rag_service.search_with_reranking.call_args[1][
                "filter_metadata"
            ]
    ...
    def test_bidirectional_linking_consistency(self):
        ...
        neo4j_structure = [{"class": {"name": "User", "file_id": file_id}}]
        ...
        with patch("neo4j.GraphDatabase.driver", return_value=self.mock_neo4j_driver):
            ...
            rag_results = self.rag_service.search_with_reranking(
                query="User model", filter_metadata={"file_id": file_id}
            )
...
class TestFileIdErrorHandling:
    ...
    def test_rag_search_with_nonexistent_file_id(self):
        ...
        results = self.rag_service.search_with_reranking(
            query="test query", filter_metadata={"file_id": "nonexistent-repo:fake.py"}
        )
    ...
    def test_malformed_file_id_handling(self):
        ...
        for malformed_id in malformed_file_ids:
            ...
            results = self.rag_service.search_with_reranking(
                query="test", filter_metadata={"file_id": malformed_id}
            )
    ...
    def test_cross_system_sync_validation(self):
        ...
        self.rag_service.search_with_reranking = Mock(
            return_value=[{"content": "class Model:", "payload": {"file_id": file_id}}]
        )
        ...
        with patch("neo4j.GraphDatabase.driver", return_value=mock_driver):
            ...
            rag_has_data = (
                len(
                    self.rag_service.search_with_reranking(
                        query="Model class", filter_metadata={"file_id": file_id}
                    )
                )
                > 0
            )
            ...
            neo4j_has_data = (
                len(
                    mock_session.run(
                        "MATCH (f:File {file_id: $file_id}) RETURN f", file_id=file_id
                    )
                )
                > 0
            )
