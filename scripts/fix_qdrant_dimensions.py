    ...
    try:
        ...
        for collection_name in target_collections:
            if collection_name in existing_collections:
                ...
                print(
                    f"INFO Collection '{collection_name}': {current_collection_dims} dimensions"
                )

                if current_collection_dims != current_dims:
                    print(
                        f"WARNING Dimension mismatch detected for '{collection_name}'!"
                    )
                    print(
                        f"   Expected: {current_dims}, Found: {current_collection_dims}"
                    )
                else:
                    print(f"OK Collection '{collection_name}' has correct dimensions")
            else:
                print(
                    f"INFO Collection '{collection_name}' not found (will be created)"
                )
        ...
        for collection_name in target_collections:
            ...
            if actual_dims == current_dims:
                ...
                print(
                    f"ERROR '{collection_name}': {actual_dims} dimensions (expected {current_dims})"
                )
                raise Exception(
                    f"Collection '{collection_name}' still has wrong dimensions!"
                )
