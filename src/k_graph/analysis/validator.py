...
class KnowledgeGraphValidator:
    ...
    async def validate_script(
        ...
        result.hallucination_indicators = self._detect_hallucinations(result)
        result.is_likely_hallucinated = len(result.hallucination_indicators) > 0
    ...
    def _detect_hallucinations(self, result: ScriptValidationResult) -> List[str]:
        ...
        for val in result.method_validations:
            if (
                ...
                if key not in reported_items:
                    ...
                    hallucination_desc = f"METHOD_NOT_FOUND at line {val.method_call.line_number}: Method '{val.method_call.method_name}' not found on class '{val.method_call.object_type}'"
                    if val.validation.suggestions:
                        hallucination_desc += (
                            f" (suggestion: {val.validation.suggestions[0]})"
                        )
                    hallucinations.append(hallucination_desc)
        ...
        for val in result.attribute_validations:
            if (
                ...
                if key not in reported_items:
                    reported_items.add(key)
                    hallucination_desc = f"ATTRIBUTE_NOT_FOUND at line {val.attribute_access.line_number}: Attribute '{val.attribute_access.attribute_name}' not found on class '{val.attribute_access.object_type}'"
                    hallucinations.append(hallucination_desc)
        ...
        for val in result.method_validations:
            if (
                ...
                hallucination_desc = f"INVALID_PARAMETERS at line {val.method_call.line_number}: Invalid parameters for method '{val.method_call.method_name}': {val.parameter_validation.message}"
                hallucinations.append(hallucination_desc)
