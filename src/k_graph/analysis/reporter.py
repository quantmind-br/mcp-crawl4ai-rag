...
from ..core.models import ScriptValidationResult, ValidationStatus
...
class HallucinationReporter:
    ...
    def generate_comprehensive_report(
        ...
        report = {
            ...
            "hallucinations_detected": validation_result.hallucination_indicators,
    ...
    def _generate_recommendations(
        ...
        kg_hallucinations = validation_result.hallucination_indicators

        if kg_hallucinations:
            method_issues = [h for h in kg_hallucinations if "METHOD_NOT_FOUND" in h]
            attr_issues = [h for h in kg_hallucinations if "ATTRIBUTE_NOT_FOUND" in h]
            param_issues = [h for h in kg_hallucinations if "INVALID_PARAMETERS" in h]
            ...
            for lib in report["libraries_analyzed"]:
                ...
                if lib["classes_used"]:
                    ...
                    for cls in lib["classes_used"]:
                        status_emoji = (
                            "VALID" if cls["status"] == "VALID" else "INVALID"
                        )
                        md.append(
                ...
                if lib["methods_called"]:
                    ...
                    for method in lib["methods_called"]:
                        status_emoji = (
                            "VALID" if method["status"] == "VALID" else "INVALID"
                        )
                        md.append(
                ...
                if lib["attributes_accessed"]:
                    ...
                    for attr in lib["attributes_accessed"]:
                        status_emoji = (
                            "VALID" if attr["status"] == "VALID" else "INVALID"
                        )
                        md.append(
                ...
                if lib["functions_called"]:
                    ...
                    for func in lib["functions_called"]:
                        status_emoji = (
                            "VALID" if func["status"] == "VALID" else "INVALID"
                        )
                        md.append(
