    ...
    async def clear_repository_data(self, repo_name: str):
        ...
        async with self.driver.session() as session:
            try:
                ...
                # Delete all repository data in one comprehensive query
                # This is more efficient and handles relationships automatically
                result = await tx.run(
                    """
                ...
                if record:
                    logger.info(
                        f"Deleted repository data: "
                        f"nodes_deleted={record.get('nodes_deleted', 0)}"
                    )
        ...
        logger.info(f"Successfully cleared data for repository: {repo_name}")
...
async def main():
    ...
    neo4j_password = os.environ.get("NEO4J_PASSWORD") or "password123"
