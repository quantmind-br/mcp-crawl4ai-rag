...
class ContextSingleton:
    ...
    async def _initialize_context(self, server: FastMCP) -> Crawl4AIContext:
        ...
        # Initialize ThreadPoolExecutor for CPU-bound operations
        cpu_count = os.cpu_count() or 1
        max_workers = min(cpu_count, 8)  # Cap at 8 for memory reasons
        cpu_executor = ThreadPoolExecutor(
            max_workers=max_workers, thread_name_prefix="ml_worker"
        )
    ...
    async def cleanup(self):
        ...
        if self._context:
            try:
                ...
                if (
                    hasattr(self._context, "cpu_executor")
                    and self._context.cpu_executor
                ):
                    logger.info("Shutting down ThreadPoolExecutor...")
                    self._context.cpu_executor.shutdown(wait=True)
